...
export const getValidToolNames = function (enabledElement) {
  const validTools = cornerstoneTools.store.state.tools.filter((tool) => tool.element === enabledElement);
  const validToolNames = validTools.map((tool) => tool.name);
  return validToolNames;
};

export const addInitialTools = function (enabledElement, tools, isStackPrefetchEnabled) {
  ...
  for (const tool of tools) {
    ...
    cornerstoneTools.addToolForElement(enabledElement, tool.toolClass, tool.props || {});
...
export const cleanupStackPrefetch = function (enabledElement, isStackPrefetchEnabled) {
  if (isStackPrefetchEnabled) {
